...
class FoodPlateActivity : BaseActivity(), FoodPlateAdapter.OnProductClickListener {
    ...
    private val foodPlateViewModel: FoodPlateViewModel by viewModels()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        newMealId = foodPlateViewModel.getLastId().inc()

        initRecyclerView()
        this.window.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN)

        foodPlateViewModel.getProductsInPlate().observe(this, {
        ...
        object : SwipeToRemove() {
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                foodPlateViewModel.updateProduct(
                    ...
                    when {
                        bottomDialogMealNameBinding.mealNameTextInput.text.isNullOrEmpty() ->
                            bottomDialogMealNameBinding.mealNameTextInputLayout.error =
                                errorEmptyMessage
                        foodPlateViewModel.checkForMealExist(
                            ...
                            runBlocking {
                                foodPlateViewModel.insertMeal(meal)
                            }

                            val listOfProducts = foodPlateAdapter.getData()

                            for (product in listOfProducts) {
                                foodPlateViewModel.updateProduct(product.apply {
                                    inFoodPlate = false
                                })

                                runBlocking {
                                    foodPlateViewModel.insertPMJoin(
                                        ProductMealJoin(
                                            product.name,
                                            meal.name
                                        )
